  ...
  if (!g || g.round >= MAX_ROUNDS) {
    if (g) {
      ...
      for (let i = 0; i < sorted.length; i++) {
        ...
        prizes.push(
          `${prize.emoji} @${id.split('@')[0]} — ${score} نقطة (+${prize.xp} XP)`
        );
      }
...
${prizes.join('\n')}`,
  ...
  const data = await fetch(
    "https://raw.githubusercontent.com/fjfilhfjjg-boop/Pomni-AI/refs/heads/main/%D8%B9%D9%8A%D9%86.md"
  ).then(r => r.json());

  const char = data[Math.floor(Math.random() * data.length)];

  const wrong = shuffle([...NAMES])
    .filter(n => n !== char.name)
    .slice(0, 3);

  const opts = shuffle([char.name, ...wrong]);

  const caption = `
╭━━━〔 👁️ خَمِّنِ الشَّخْصِيَّة 〕━━━╮
┃ 📊 الجولة: ${g2.round} / ${MAX_ROUNDS}
╰━━━━━━━━━━━━━━━━━━━━╯
